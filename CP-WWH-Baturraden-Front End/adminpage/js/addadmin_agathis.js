document.addEventListener('DOMContentLoaded', () => {
    ...
    // ID Villa diubah menjadi 2 untuk Agathis
    const VILLA_ID = 2; 
        ...
        roomTypes.forEach(room => {
            ...
            roomItem.innerHTML = `
                <div class="room-option-details">
                    <h6>${room.name}</h6>
                    <p class="sub-info"><i class="fas fa-info-circle"></i> ${room.is_all_in ? `Seluruh unit Villa (${room.total_rooms} Kamar Tidur)` : `Kamar untuk ${room.capacity_per_room} orang`}</p>
                </div>
                <div class="room-option-capacity"><i class="fas fa-users"></i> ${room.capacity_per_room}</div>
    ...
    function highlightActiveSidebarLink() {
        ...
        navLinks.forEach(li => {
            ...
            // Cek apakah href ada dan cocok dengan path terakhir
        ...
        roomOptionsList.addEventListener('click', (event) => {
            if (event.target.classList.contains('btn-select-room')) {
                ...
                if (selectedRoomId) {
                    ...
                    // Menghapus alert agar alur lebih cepat
